...
export function ApplicantGovRefDetails({ visaApplication, onSuccess }: ApplicantGovRefDetailsProps) {
    const { personalInfo, additionalApplicants } = visaApplication;
    const [activeTab, setActiveTab] = useState("primary");

    return (
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className={`grid grid-cols-${Math.min(6, additionalApplicants.length + 1)} mb-4`}>

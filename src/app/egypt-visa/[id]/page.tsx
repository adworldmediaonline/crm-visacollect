...
import { ArrivalInfoCard } from '@/components/egypt-visa/ArrivalInfo';
import { VisaDetailsCard } from '@/components/egypt-visa/VisaDetails';
import { ApplicationStatusCard } from '@/components/egypt-visa/ApplicationStatus';
import { DeclarationCard } from '@/components/egypt-visa/Declaration';
import { ApplicantTabs } from '@/components/egypt-visa/ApplicantsTabs';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
const EgyptVisaDetailPage = () => {
  const params = useParams();
  const id = params.id as string;
  const [isReminderSending, setIsReminderSending] = useState(false);

  const { data, isLoading, error } = useQuery({
    queryKey: ['EgyptVisaApplication', id],
    queryFn: async () => {
      const response = await egyptVisaApi.getApplicationById(id);
  ...
  const documentReminderMutation = useMutation({
    mutationFn: () => egyptVisaApi.sendDocumentReminder(id),
    onMutate: () => {
  ...
  const paymentReminderMutation = useMutation({
    mutationFn: () => egyptVisaApi.sendPaymentReminder(id),
    onMutate: () => {
      setIsReminderSending(true);
    },
    onSuccess: () => {
  ...
  const passportReminderMutation = useMutation({
    mutationFn: () => egyptVisaApi.sendPassportReminder(id),
    onMutate: () => {
      setIsReminderSending(true);
    },
    onSuccess: () => {
  ...
  const photoReminderMutation = useMutation({
    mutationFn: () => egyptVisaApi.sendPhotoReminder(id),
    onMutate: () => {
      setIsReminderSending(true);
    },
    onSuccess: () => {
...
export default EgyptVisaDetailPage;
